...
/**
 * @brief Read a single fuzzing test case from standard input.
 *
 * SDFuzz/AFL stdin mode provides the current test case on STDIN.  The server
 * demo normally receives a single BACnet NPDU from datalink_receive(); this
 * helper provides the same single-buffer input source without requiring the
 * AFL @@ file placeholder.
 */
static size_t stdin_read_test_case(uint8_t *buffer, size_t max_len)
{
    size_t total = 0;
    size_t count = 0;

    while (total < max_len) {
        count = fread(&buffer[total], 1, max_len - total, stdin);
        if (count == 0) {
            break;
        }
        total += count;
...
/**
 * @brief Convert stdin bytes into the NPDU buffer expected by npdu_handler().
 *
 * The original BACnet/IP datalink path strips the BVLC wrapper before calling
 * npdu_handler().  For compatibility with both existing BACnet/IP packet seeds
 * and raw NPDU seeds, this function accepts:
 *   - raw NPDU bytes; or
 *   - BACnet/IP BVLC frames carrying an Original/Forwarded NPDU.
 */
static uint16_t stdin_prepare_npdu(uint8_t *buffer, size_t input_len)
{
    size_t npdu_offset = 0;
    size_t npdu_len = input_len;
    uint16_t bvlc_len = 0;
    ...
    /* BACnet/IP BVLC: type=0x81, function, two-byte big-endian length. */
    if ((input_len >= 4) && (buffer[0] == 0x81)) {
        bvlc_len = (uint16_t)(((uint16_t)buffer[2] << 8) | buffer[3]);
        if ((bvlc_len < 4) || (bvlc_len > input_len)) {
            return 0;
        }
        ...
        switch (buffer[1]) {
            ...
            case 0x0B: /* Original-Broadcast-NPDU */
                npdu_offset = 4;
                break;
            case 0x04: /* Forwarded-NPDU: BVLC + originating B/IP address */
                if (bvlc_len < 10) {
                    return 0;
                }
                npdu_offset = 10;
        ...
        if (bvlc_len <= npdu_offset) {
            return 0;
        }
        npdu_len = (size_t)bvlc_len - npdu_offset;
        memmove(buffer, &buffer[npdu_offset], npdu_len);
    }

    if (npdu_len > UINT16_MAX) {
        return 0;
    }

    return (uint16_t)npdu_len;
}
...
int main(int argc, char *argv[])
{
...
#endif
    /*
     * SDFuzz/AFL stdin mode: process exactly one test case from STDIN and exit.
     * This replaces the blocking datalink_receive() loop while preserving the
     * server demo initialization and the normal npdu_handler() processing path.
     */
    ...
    pdu_len = stdin_prepare_npdu(&Rx_Buf[0], stdin_read_test_case(&Rx_Buf[0], MAX_MPDU));
    if (pdu_len) {
